...
describe('festival-audio', () => {
  const loadSets = ({ addSeconds = 5, alternate = false } = {}) => {
    ...
    const data = {
      sets: [
        {
          audio: `../test/components/festival-audio/striding-along.mp3?${
            alternate ? 3 : 1
          }`,
        ...
        },
        {
          ...
          start: add(now, addSeconds + 110).toISOString(),
          length: 106,
        },
      ],
    };
  ...
  const commonTests = (template) => {
    ...
    describe('before the show', () => {
      ...
      it('fires loadedmetadata event with set and duration', async () => {
        ...
        expectNumbersAreAlmostEqual(detail.duration, 106);
      });
    ...
    describe('during the show', () => {
      ...
      it('preloads the next set 60 seconds before the end of the first set', async () => {
        const SET_LENGTH = 106;
        store.dispatch(loadSets({ addSeconds: -SET_LENGTH + 62 }));
        store.dispatch(tick());
      ...
      it('does not update src if set info is changed', async () => {
        const SET_LENGTH = 106;
        store.dispatch(loadSets({ addSeconds: -SET_LENGTH + 2 }));
        store.dispatch(tick());
