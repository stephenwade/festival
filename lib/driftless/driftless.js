...
// modified to add new function to run a function every second on the second
...
export function setDriftlessIntervalEverySecond(fn, ...params) {
  const callFn = castToFn(fn);
  let id;
  const thisSecond = () => {
    const nowMs = Date.now();
    const soonMs = nowMs + 100;
    return Math.trunc(soonMs / 1000) * 1000;
  };
  const opts = {
    atMs: thisSecond() + 1000,
    customNow: Date.now,
    fn(...args) {
      opts.atMs += 1000;
      tryDriftless(id, opts);
